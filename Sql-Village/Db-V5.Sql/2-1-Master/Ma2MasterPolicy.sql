...
-----------------------------------------------------------------------------------------------------------------------
-- Drop Table Ma2MasterPolicy   -- 村里長政見
-----------------------------------------------------------------------------------------------------------------------
CREATE TABLE Ma2MasterPolicy
(
	--AutoId         int IDENTITY(1,1) NOT NULL,
	Id int IDENTITY(1,1) NOT NULL,
	TeamId NVARCHAR(100) NOT NULL,--村里代號
	OrderNo decimal(10,2) NOT NULL,	-- 次序
	OrderTitle NVARCHAR(50) ,	-- 序號
	Descriptions NVARCHAR(1000) NOT NULL,	-- 政見
	Notes NVARCHAR(200) ,--備註說明

	-- 以下每檔資料表都會有這些欄位
	WriteInfo NVARCHAR(100),
	CONSTRAINT Ma2MasterPolicy_PrimaryKey PRIMARY KEY CLUSTERED(Id),
	CONSTRAINT Ma2MasterPolicy_ref_Ma1Master FOREIGN KEY (TeamId) REFERENCES Ma1Master (TeamId) ON UPDATE CASCADE ON DELETE CASCADE
);
Go

--建立索引檔
--CREATE UNIQUE CLUSTERED INDEX Ma2MasterPolicy_AutoId ON Ma2MasterPolicy(AutoId);
--go
CREATE INDEX Ma2MasterPolicy_TeamId ON Ma2MasterPolicy(TeamId,OrderNo);
Go

-- DROP TRIGGER Ma2MasterPolicy_Trigger1
go
CREATE TRIGGER Ma2MasterPolicy_Trigger1 ON Ma2MasterPolicy AFTER UPDATE,DELETE NOT FOR REPLICATION AS
Begin
	...
	Values('Ma2MasterPolicy',@tableKey,@writeType,@newData,@oldData);
End
Go