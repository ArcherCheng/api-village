-----------------------------------------------------------------------------------------------------------------------
-- Drop Table Ma2MasterEducation    -- 村里長學歷
-----------------------------------------------------------------------------------------------------------------------
CREATE TABLE Ma2MasterEducation
(
	...
	CONSTRAINT Ma2MasterEducation_PrimaryKey PRIMARY KEY CLUSTERED(Id),
	CONSTRAINT Ma2MasterEducation_ref_Ma1Master FOREIGN KEY (TeamId) REFERENCES Ma1Master (TeamId) ON UPDATE CASCADE ON DELETE NO ACTION
);
Go

--建立索引檔
--CREATE UNIQUE CLUSTERED INDEX Ma2MasterEducation_AutoId ON Ma2MasterEducation(AutoId);
--go
CREATE INDEX Inx_TeamId ON Ma2MasterEducation(TeamId,OrderNo);
Go

-- DROP TRIGGER Ma2MasterEducation_Trigger1
go
CREATE TRIGGER Ma2MasterEducation_Trigger1 ON Ma2MasterEducation AFTER UPDATE,DELETE NOT FOR REPLICATION AS
Begin
	...
	Values('Ma2MasterEducation',@tableKey,@writeType,@newData,@oldData);
End
Go