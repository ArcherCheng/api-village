...
BEGIN
	...
	SET @xdate = Cast(@xyear + '-' + @xmonth + '-' + @xday as Date);
...
CREATE FUNCTION dbo.YYYYMMToLastDate (@strMonth nvarchar(6))
RETURNS Date
AS
BEGIN
	DECLARE @xyear CHAR(4);
	...
	DECLARE @xmonth CHAR(2);
	SET @xmonth = SUBSTRING (@strMonth,5,2);

	DECLARE @xdate datetime;
	--SET @xdate = convert(datetime, @xyear + '-' + @xmonth + '-01' , 111);
	SET @xdate = Cast(@xyear + '-' + @xmonth + '-01' as date);
...
CREATE FUNCTION dbo.YYYYMMToFirstDate (@strMonth nvarchar(6))
RETURNS Date
AS
BEGIN
	DECLARE @xyear CHAR(4);
...
go
CREATE FUNCTION dbo.UpperFirstChar (@strWord NVARCHAR(500))
